...
func loadRules(fname string) ([]*Rule, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Rule parsing error %v", err)
	}
	return rl, nil
}
...
func setupRules(baserules []*Rule) ([]*Rule, bool) {
	...
	warnonce("Problem loading Rules file %s: %s\n", rfile, err)
...
func main() {
	...
	flag.BoolVar(&failgotdata, "M", false, "Reject all (after messages are fully received)")
	flag.BoolVar(&goslow, "S", false, "Send output to the network slowly (10 chars/s)")
	flag.StringVar(&srvname, "helo", "", "Server `hostname` for greeting banners")
	flag.StringVar(&smtplogfile, "smtplog", "", "Log all SMTP exchanges to FILE ['-': to stdout]")
	flag.StringVar(&dnlogfile, "dnlog", "", "Log all do-nothing client connections to FILE ['-': to stdout]")
	flag.StringVar(&logfile, "l", "", "Log info on each received email to FILE ['-': to stdout]")
	flag.StringVar(&savedir, "d", "", "DIRECTORY to save received messages")
	flag.BoolVar(&force, "force-receive", false, "Mails are accepting even without a '-d' directory")
	flag.StringVar(&hashtype, "save-hash", "all", "Saved messages based on hash of: full/")
	flag.StringVar(&certfile, "C", "", "TLS PEM certificate CERTFILE [requires -K as well]")
	flag.StringVar(&keyfile, "K", "", "TLS PEM key KEYFILE [requires -C as well]")
	...
	flag.StringVar(&rfiles, "r", "", "FILE(s) [comma-separated] of Control Rules")
	...
	flag.StringVar(&connfile, "c", "", "FILE of configuration for -helo, -C and -K arguments")
	...
	if flag.NArg() == 0 {
		fmt.Fprintf(os.Stderr, "%s: No [HOST]:PORT argument(s) given about what to listen on\n", self)
		fmt.Fprintf(os.Stderr, "Usage:  %s [OPTIONS] [HOST]:PORT...\n", self)
		return
	}

	if savedir == "" && !(force || failgotdata) {
		die("Not accepting mail without either a '-d DIRECTORY' or a '-force-receive'\n")
	}

	if !(hashtype == "msg" || hashtype == "full" || hashtype == "all") {
		die("Invalid option for '-save-hash': '%s'. Must be 'msg'/'full'/'all'\n", hashtype)
	}

	if yakCount > 0 && smtplogfile == "" {
		die("Flag '-dncount' requires '-smtplog'\n")
	}

	if yakCount > 0 && yakTimeout < time.Second {
		die("Argument for '-dndur' must be at least one second\n")
	}

	if dnlogfile != "" && yakCount == 0 {
		die("Flag '-dnlog' requires '-dncount'")
	}
	...
	case certfile != "" && keyfile != "":
		...
		if err != nil {
			die("Error loading TLS cert(s) & key(s) from %s & %s: %v\n", certfile, keyfile, err)
		}

	case certfile != "":
		die("Certfile(s) specified without any keyfile\n")

	case keyfile != "":
		die("Keyfile(s) specified without any certfile\n")
	}
	...
	if err != nil {
		die("Error opening message logfile '%s': %v\n", logfile, err)
	}
	...
	if !isgood {
		...
		warnonce("")
		die("Aborting, problems in the Rules files\n")
	}
	...
	case "from", "to", "data", "message", "accepted", "helo":
		// it's okay
	default:
		die("Invalid -minphase argument '%s': must be 'helo'/'ehlo'/'from'/'to'/'data'/'message'/'accepted'\n", minphase)
	}
	...
	if pprofserv != "" {
		...
		go func() {
			...
			if e != nil {
				die("HTTP server for pprof failed: %s\n", e)
			}
		}()
	}
